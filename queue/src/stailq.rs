...
/// A singly-linked list is headed by a forward pointer and a tail pointer. The
/// elements are singly linked for minimum space and pointer manipulation
/// overhead at the expense of O(n) removal for arbitrary elements. A
/// singly-linked tail queue may only be traversed in the forward direction.
...
impl<T> STailQ<T> {
    ...
    pub fn split_off(&mut self, at: usize) -> STailQ<T> {
        let len = self.len();
        ...
        let new_head = unsafe { (*node.as_ptr()).next };

        self.tail = Some(node);

        unsafe { (*self.tail.as_ref().unwrap().as_ptr()).next = None };
...
impl<T> IntoIterator for STailQ<T> {
    type Item = T;
...
impl<'a, T> IntoIterator for &'a STailQ<T> {
    type Item = &'a T;
...
impl<'a, T> IntoIterator for &'a mut STailQ<T> {
    type Item = &'a mut T;
...
impl<T: PartialEq> PartialEq for STailQ<T> {
...
impl<T: Eq> Eq for STailQ<T> {}

impl<T: PartialOrd> PartialOrd for STailQ<T> {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        self.iter().partial_cmp(other)
    }
}

impl<T: Ord> Ord for STailQ<T> {
    #[inline]
...
unsafe impl<T: Send> Send for STailQ<T> {}
unsafe impl<T: Sync> Sync for STailQ<T> {}
