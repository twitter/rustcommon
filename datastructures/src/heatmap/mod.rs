    ...
    u64: From<<T as Atomic>::Primitive>,
{
    ...
    fn get_index(&self, time: u64) -> Option<usize> {
        if self.oldest_begin_precise.load(Ordering::Relaxed) < time
            && time < self.newest_end_precise.load(Ordering::Relaxed)
        {
            let mut index = ((time - self.oldest_begin_precise.load(Ordering::Relaxed))
                / self.resolution.load(Ordering::Relaxed)) as usize
    ...
    fn tick(&self) {
        self.slices[self.offset.load(Ordering::Relaxed) as usize].clear();
        if self.offset.load(Ordering::Relaxed) as usize == (self.slices.len() - 1) {
            self.offset.store(0, Ordering::Relaxed);
        } else {
            self.offset.fetch_add(1, Ordering::Relaxed);
        }
        self.oldest_begin_precise
            .fetch_add(self.resolution.load(Ordering::Relaxed), Ordering::Relaxed);
        self.newest_begin_precise
            .fetch_add(self.resolution.load(Ordering::Relaxed), Ordering::Relaxed);
        self.newest_end_precise
            .fetch_add(self.resolution.load(Ordering::Relaxed), Ordering::Relaxed);
        let current = *self.oldest_begin_utc.lock().unwrap();
        *self.oldest_begin_utc.lock().unwrap() =
            current + time::Duration::nanoseconds(self.resolution.load(Ordering::Relaxed) as i64);
    }
    ...
    /// ```
    pub fn increment(&self, time: u64, value: u64, count: <T as Atomic>::Primitive) {
    ...
    /// ```
    pub fn decrement(&self, time: u64, value: u64, count: <T as Atomic>::Primitive) {
        if let Some(index) = self.get_index(time) {
    ...
    pub fn latch(&self) {
        ...
        if time >= self.newest_begin_precise.load(Ordering::Relaxed) {
            let ticks = (time - self.newest_begin_precise.load(Ordering::Relaxed))
