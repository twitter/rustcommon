...
where
    ...
    <T as Atomic>::Primitive: Default + PartialEq + Copy + Into<u64>,
{
    buckets: Vec<T>,
...
where
    T: Atomic + SaturatingArithmetic + Unsigned + Default + FetchCompareStore,
    <T as Atomic>::Primitive: Default + PartialEq + Copy + Into<u64>,
{
    ...
    pub fn with_limit(limit: u64, alpha: f64) -> Result<Self, DDSketchError> {
        ...
        //    γ = (1 + α) / (1 - α)
        // This is the ratio between the maximum and minimum value
        // within a bucket.
        //
    ...
    pub fn increment(&self, value: u64, count: <T as Atomic>::Primitive) {
        ...
        self.count
            .fetch_saturating_add(count.into(), Ordering::Relaxed);

        self.buckets[self.index_of(value)].fetch_saturating_add(count, Ordering::Relaxed);
    }
    ...
    /// calculation may end up being inaccurate if the sketch is
    ...
    pub fn merge(&self, other: &Self) -> Result<(), DDSketchError> {
        ...
        self.buckets
            ...
            .for_each(|(x, y)| {
                x.fetch_saturating_add(y.load(Ordering::Relaxed), Ordering::Relaxed);
            });

        self.count
            .fetch_saturating_add(other.count(), Ordering::Relaxed);
        self.min
