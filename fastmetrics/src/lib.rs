    ...
    T: Metric,
{
    ...
    pub fn get_counter(&self, metric: T) -> u64 {
        match self.get(metric) {
            ...
            Some(Channel::Counter { counter, .. }) => counter.load(Ordering::Relaxed),
            _ => {
    ...
    pub fn get_gauge(&self, metric: T) -> i64 {
        match self.get(metric) {
            ...
            Some(Channel::Gauge { gauge, .. }) => gauge.load(Ordering::Relaxed),
            _ => {
...
mod tests {
    ...
    fn counter() {
        let metrics = MetricsBuilder::new()
            .counter(Metric::Alpha)
            .counter(Metric::Charlie)
            .build();
